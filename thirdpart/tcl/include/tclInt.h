...
typedef Tcl_CollectionElem CollectionElem;
typedef Tcl_Collection Collection;
...
// Added by Ling Wang @ 18:02:05 2007-10-22
// ------------------------------------------------------------------------
extern int stdinFd[], stdoutFd[], stderrFd[];
extern int redirectedInputFd[], redirectedOutputFd[], redirectedErrorFd[];
extern int stdinRedirected[], stdoutRedirected[], stderrRedirected[];
// ------------------------------------------------------------------------
...
EXTERN Tcl_ObjType	tclCollectionType;

EXTERN int TclCompileNoOp _ANSI_ARGS_((Tcl_Interp *interp, Tcl_Parse *parsePtr, struct CompileEnv *envPtr));
EXTERN int ProcessProcResultCode _ANSI_ARGS_((Tcl_Interp *interp, char *procName, int nameLen, int returnCode));
...
EXTERN int		TclParseInteger _ANSI_ARGS_((CONST char *string,
			    int numBytes));
// Added by pengxj to set the a channel to time.log
EXTERN void             Tcl_SetTimeLogChan _ANSI_ARGS_((char* timeLogFilePath));
// Added by Ling Wang @ 17:57:17 2007-10-22
// ------------------------------------------------------------------------
EXTERN int    tclCmdPreprocess _ANSI_ARGS_((Tcl_Interp *interp,
          int objc, Tcl_Obj *CONST objv[], int *prepObjc, Tcl_Obj* prepObjv[]));
// ------------------------------------------------------------------------
EXTERN int    restoreIO(Tcl_Interp *interp);
