...
# Modified by Ling Wang @ 11:04:23 2008-01-29
# unknown --
...
#       3. See if the command exists as an executable UNIX program. If so,
#          "exec" the command.
#       4. See if the command requests csh-like history substitution in one
#          of the common forms !!, !<number>, or ^old^new.  If so, emulate
#          csh's history substitution.
#       5. See if the command is a unique abbreviation for anothe command.
#          If so, invoke the command.
#
# Arguments:
...
proc unknown args {
    ...
    # Added by Ling Wang @ 17:01:34 2008-05-08
    global cmd_system_hint
...
#     }
    ...
    if {[llength $cmds] == 1} {
        if {!(([info level] == 1) && [info script] eq "" \
                  && [info exists tcl_interactive] && $tcl_interactive)
            && [info exists cmd_system_hint] && [string is true $cmd_system_hint]} {
            puts "HINT: it is more efficient to use a complete command name than to let command system guess it from a stub, and the complete command name for `$name' is `[lindex $cmds 0]'. HINT can be turned off by setting `cmd_system_hint' to false."
        }
