...
namespace ece {
  ...
  void eceReadLineLoop(Tcl_Interp* interp);
  ...
  char* eceCommandsGenerator(const char* text, int state) {
    ...
    int num_ece_cmds = gCommands.numCmds();
    int num_cmds = num_ece_cmds + num_internal_cmds;
    while (list_index < num_cmds) {
      if (list_index < num_ece_cmds) {
        name = gCommands.commands(list_index).c_str();
      } else {
        name = internal_cmds[list_index - num_ece_cmds];
      }
      list_index++;
  ...
  char* eceCommandsOptionGenerator(const char* text, int state) {
    static int list_index;
    static size_t len;
    if (text == NULL || text[0] != '-')
      return ((char *)NULL);
  ...
  char* eceTclVarGenerator(const char* text, int state) {
    static int list_index;
    static size_t len;
    static Tcl_Obj** objv;
    static int objc = 0;
  ...
  char** eceCommandsCompletion(const char* text, int start, int end) {
    char **matches;
    matches = (char **)NULL;
    if (start == 0)
      matches = rl_completion_matches(text, eceCommandsGenerator);
    else if (text && text[0] == '-')
      matches = rl_completion_matches(text, eceCommandsOptionGenerator);
    else if (text && text[0] == '$')
      matches = rl_completion_matches(text, eceTclVarGenerator);
    return matches;
  }
  void eceReadLineInit(const char* history_file) {
    rl_readline_name = "ece_tclreadline";
    ...
    rl_attempted_completion_function = eceCommandsCompletion;
  ...
  void eceReadLineLoop(Tcl_Interp* interp) {
#ifndef WIN32
    const char* prompt = "ece>";
    tclrl_interp = interp;
    const char* history_file = ".ece_history_cmd";
    eceReadLineInit(history_file);
#endif
    if (init_tcl_file) {
