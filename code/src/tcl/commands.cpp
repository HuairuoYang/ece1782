...
namespace ece {
  ...
  bool Commands::postRun(int objc, Tcl_Obj* const objv[]) {
    ...
    if (verbose_) {
      ...
      usleep(6000);
#endif
      ece_print_cmd_profile(cmd_name, time_begin_, time_end_, tp_begin_, tp_end_);
      int cmd_used_mem = DataVar::cmd_peak_used_memory_ - memory_begin_;
      ece_info("*INFO*: %s used memory %dMb, ece used peak memory %dMb, current used memory %dMb.\n", cmd_name, cmd_used_mem, DataVar::ece_peak_used_memory_, DataVar::current_used_memory_);
    }
    decr_output_indent();
