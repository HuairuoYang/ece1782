...
namespace ece {
  ...
  bool App::canonicalFilePath(std::string & file_path, std::string &canonical_path)
  {
    ...
    if (strlen(file_path.data()) > MAX_PATH || PathCanonicalizeW(lpBuffer, wc_path) == FALSE) {
      ece_error("*ERROR*: Get real-path failed for path '%s', check the file path length and format.\n", file_path.data());
      return false;
    }
    if (PathFileExists(lpBuffer) == FALSE) {
      ece_error("*ERROR*: File '%s' not exist.\n", file_path.data());
      return false;
    }
    // wchar_t => std::string
    ...
    if (_fullpath(full_path, canonical_path.data(), MAX_PATH) == NULL) {
      ece_error("*ERROR*: Get full-path failed for path '%s'.\n", canonical_path.data());
      return false;
    }
    ...
    if (strlen(path) > MAX_PATH || FALSE == PathCanonicalize(l_path, path)) {
      ece_error("*ERROR*: Get real-path failed for path '%s'.\n", file_path.data());
      return false;
    }
    char full_path[MAX_PATH];
  ...
  std::string App::getDBPath() {
    ...
    char ece_path[1024] = { 0 };
    if (readlink("/proc/self/exe", ece_path, sizeof(ece_path)) <= 0)
      return "";
#else
    std::string key_bin("\\bin\\");
    std::string key_to_replace("\\db");
    char ece_path[MAX_PATH] = { 0 };
    wchar_t ece_path_wchar[MAX_PATH] = { 0 };
    HINSTANCE hInst = GetModuleHandle(NULL);
    GetModuleFileName(hInst, ece_path_wchar, sizeof(ece_path_wchar));
    wcstombs(ece_path, ece_path_wchar, sizeof(ece_path));
#endif // !WIN32

    char* ece_db_dir = NULL;
    std::string exec_path = ece_path;
    ...
    if (found != std::string::npos) {
      ...
      if (!File::access(exec_path.c_str(), 0)) {
        ece_db_dir = strdup(exec_path.c_str());

      }
    }

    if (ece_db_dir == NULL) {
      ece_db_dir = strdup(getenv("ece_DB"));
    }

    if (ece_db_dir != NULL) {
      std::string dir = ece_db_dir;
      free(ece_db_dir);
      return dir;
    }
  ...
  bool App::avoidOverwrite(const std::string & file_name) {
    ...
    if ((pDir = opendir(dir_path.c_str())) != NULL) {
      ...
      ece_error("*ERROR*: The directory '%s' does not exist.\n", dir_path.c_str());
    }
#else
    ...
    for (size_t i = 0; i < all_files.size(); i++) {
      ...
      if (found == std::string::npos) {
        ece_warning("*WARNING*: Found a file without suffix: '%s'.\n");
        continue;
      }
    ...
    if (max >= 0) {
      ...
      ece_info("*INFO*: The existing file '%s' was renamed to '%s' to avoid overwriting.\n", file_name.c_str(), new_file_name.c_str());
    }
    return true;
  }
}
