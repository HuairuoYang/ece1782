...
namespace ece {
  ...
  void ece_print_cmd_profile(const char* cmd_name, const timeval time_begin, const timeval time_end, const timespec tp_begin, const timespec tp_end) {
    ...
    if (elapsed_time > 0.01) {
      ece_info("*INFO*: %s elapsed time %.2f seconds, cpu_time %.2f seconds\n", cmd_name, elapsed_time, cpu_time);
    }
    ...
    if (!proc_status_stream.good()) {
      ...
      while (proc_status_stream.getline(buf, MAX_SIZE)) {
        if (!strncmp(buf, "VmRSS", 5)) {
          ...
          ece_assert(split_strings.size() > 1);
  ...
  void dump_stack() {
#ifdef WIN32
    ece_info("*INFO*: Dumping stack is not supported in windows.\n");
#else
    void *buffer[1024];
    ...
    if (readlink("/proc/self/exe", exec_path, sizeof(exec_path)) <= 0)
      ece_info("Could not get the execute binary's path!\n");
    char cmd[2048];
